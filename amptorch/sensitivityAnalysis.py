...
def loss_curve(net, label, loss_critera=['train_loss', 'valid_loss'], extra_label=None):
    """
    ...
    if extra_label is None: 
        ...
        fig.savefig('results/plots/loss_curve/loss_' + label + '_' + extra_label + '.pdf')


def train_test_analysis(calc, images_train, label, images_test=None, data="energy", legend=None):
    """
        ...
        for images_collect in [images_train, images_test]:
            if images_collect is not None:
                ...
                if images_collect == images_train:
                    ax.scatter(targets, preds, marker='v', s=7, alpha=.6, label=labels[0])
                    # print set RMSE
                    rmse = np.sqrt(((targets - preds)**2).sum()/len(preds))
                    rmse_dict['training RMSE'] = rmse
                    print('Energy RMSE of training set = {:.4f} eV per atom.'.format(rmse))
                elif images_collect == images_test:
                    ax.scatter(targets, preds, marker='o', s=7, alpha=.8, label=labels[1])
                    # print set RMSE
                    rmse = np.sqrt(((targets - preds)**2).sum()/len(preds))
                    rmse_dict['testing RMSE'] = rmse
                    print('Energy RMSE of testing set = {:.4f} eV per atom.'.format(rmse))
...
def subsample_analysis(calc, images_train, label, images_whole=None, images_test=None, data="energy", legend=None):
    """
    Train the neural network model with testing sets separated from training dataset. 
    Input: 
        calc: AMP class
        ...
        for images_collect in [images_whole, images_train, images_test]:
            if images_collect is not None:
                ...
                if images_collect == images_whole:
                    ax.scatter(targets, preds, marker='v', s=7, alpha=.4, label=labels[0])
                    # print set RMSE
                    rmse = np.sqrt(((targets - preds)**2).sum()/len(preds))
